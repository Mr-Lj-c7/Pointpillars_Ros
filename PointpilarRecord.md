...
## OpenPCDet Version: 0.6.0 (本项目对应版本)
...
3.1 **kitti数据集下载:**[kitti数据解析](https://blog.csdn.net/zyw2002/article/details/127395975) ；下载后的数据集需要放在指定路径下，具体参考：[OpenPCDet官方文档](https://github.com/open-mmlab/OpenPCDet/blob/master/docs/GETTING_STARTED.md)


###   3.2 **预处理:**

3.2.1 根据自己需求，调整点云范围，需要注意的是x和y轴范围与voxel_size的比值需要是16的倍数，eg:（69.12+69.12）/0.16 /16= 54

3.2.2 kitti数据预处理(数据打包pkl，加速运算)
...
3.3.1 **训练路径:**/home/user/ChangTing/Code/Openpcdet/OpenPCDet/tools/

3.3.2 **conda 环境:**python_3.9
...
3.4.1 **验证路径:**/home/user/ChangTing/Code/Openpcdet/OpenPCDet/tools/
  使用训练好的pth模型验证数据集
...
3.5.1 **推理路径:**/home/user/ChangTing/Code/Openpcdet/OpenPCDet/tools/
  使用pth模型推理单帧数据
...
3.6.1 **pth to onnx 路径:**/home/user/ChangTing/Code/Openpcdet/Lidar_AI_Solution/CUDA-PointPillars/tool/

3.6.2 **conda 环境:**pointpillars
...
3.7.1 **pth to onnx 路径:**/home/user/ChangTing/Code/Openpcdet/OpenPCDet/tools/export_onnx/

3.7.2 **conda 环境:**pointpillars
...
3.8.1 [说明]自定义模型进行训练，需要修改pointpillar.yaml文件,建议复制文件修改相关参数，如：检测范围、模型结构（体素数量、体素大小、先验框参数配置等）、数据集、检测类别等.在对训练好的pytorch模型进行ONNX转换时，需要根据pointpillar.yaml文件修改模型的输入、输出张量，具体见[simplifier_onnx.py]。


###   3.9 **模型部署:**

3.9.1 **模型部署路径:**/home/user/ChangTing/Code/Openpcdet/PointPillars_Ros/ 

3.9.2 [说明]基于CUDA、TensorRT部署pointpillar模型,结合ROS1进行3D目标检测并发布车辆坐标系下的目标检测结果，共有[autoware_msgs]、[detected_objects_visualizer]、[lidar_point_pillars]3个功能包、[部署参考](https://blog.csdn.net/h904798869/article/details/132411664),验证自定义模型范围时请修改Pointpillars/params.h文件中的MARK参数。
...
## 6. ubuntu20.04 网卡驱动问题

6.1 [网卡驱动问题](https://blog.csdn.net/weixin_52490336/article/details/133139105)
