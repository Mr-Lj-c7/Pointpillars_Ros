...
private:
  ...
  const double object_max_linear_size_ = 50.f;
  double object_speed_threshold_;
  double arrow_speed_threshold_;
  double marker_display_duration_;

  int marker_id_, tracker_marker_id_;

  std_msgs::ColorRGBA label_color_, box_color_, hull_color_, arrow_color_, centroid_color_, model_color_, tracker_color_;

  std::string input_topic_, ros_namespace_;

  ros::NodeHandle node_handle_;
  ros::Subscriber subscriber_detected_objects_;
  ros::Subscriber subscriber_tracker_objects_;

  ros::Publisher publisher_markers_;
  ros::Publisher publisher_tracker_markers_;
  ...
  void TrackerObjectsCallback(const autoware_msgs::DetectedObjectArray &in_objects);

  visualization_msgs::MarkerArray TrackerObjectsToCentroids(const autoware_msgs::DetectedObjectArray &in_objects);
  visualization_msgs::MarkerArray TrackerObjectsToLabels(const autoware_msgs::DetectedObjectArray &in_objects);
