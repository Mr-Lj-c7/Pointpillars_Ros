...
class TRT {
  private:
    ...
    nvinfer1::IExecutionContext *context_ = nullptr;  // TensorRT 执行上下文负责管理和执行推理任务
...
class PointPillar {
  private:
    ...
    std::shared_ptr<PreProcessCuda> pre_;    // 预处理
    std::shared_ptr<TRT> trt_;               // trt模型推理
    std::shared_ptr<PostProcessCuda> post_;  // 后处理
  ...
  public:
    ...
    ~PointPillar(void);
    int doinfer(void*points, unsigned int point_size, std::vector<Bndbox> &res);  // 推理
};
